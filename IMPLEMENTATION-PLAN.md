...
> 基于三 AI 4 轮辩论共识方案拆分 | 2026-06-14 | 16 个任务 | 预计 20 天
...
#6  [ ] 状态持久化         ──── 阻塞于 #3
     │
Phase 1-B: 浏览器层（严格串行）─────────────────────────────────────────
#7  [ ] PlaywrightManager   ──── 需 #2
#8  [ ] ProfileManager      ──── 阻塞于 #7
#9  [ ] PlatformAdapter接口 ──── 阻塞于 #8
#10 [ ] 三平台适配器实现    ──── 阻塞于 #9
     │
Phase 2-3: 辅助组件 ───────────────────────────────────────────────────
#11 [ ] ConversationManager  ──── 阻塞于 #10
     │
Phase 3: 核心编排 ─────────────────────────────────────────────────────
#12 [ ] DebateOrchestrator   ──── 阻塞于 #3 + #10 + #5  ← 🔥 最关键
     │
Phase 4-5: 输出 + API ─────────────────────────────────────────────────
#13 [ ] 报告生成器           ──── 阻塞于 #12 + #6
#14 [ ] per-platform 队列    ──── 阻塞于 #12
#15 [ ] REST API + 异常处理  ──── 阻塞于 #12 + #13
     │
Phase 6: 验证 ─────────────────────────────────────────────────────────
#16 [ ] 端到端测试           ──── 阻塞于 #15 + #14
```
...
| **补：Lucene 依赖** | ⬜ 待补 | `org.apache.lucene:lucene-core` + `lucene-analysis-common` 9.x |
...
**待办：** 在 `pom.xml` 的 `<dependencies>` 中补充：

```xml
<!-- Apache Lucene（TF-IDF 文本预处理与相似度） -->
<dependency>
...
> 📝 基于 05-final-synthesis.md 共识方案生成。实施中如有偏差以代码实际运行结果为准。
